...
async def register(
    db: AsyncSession, data: UserCreateDTO, repo: IUsersRepository
) -> TokenDTO:
    _validate_credentials(data.username, data.password)
    if await repo.exists_by_username(db, data.username):
        raise EntityConflictError("Username already registered.")
    user = await repo.create(db, data.username, _hash_password(data.password))
    return user_to_token_dto(user, _create_token(user.id, user.username))


async def login(db: AsyncSession, data: LoginDTO, repo: IUsersRepository) -> TokenDTO:
