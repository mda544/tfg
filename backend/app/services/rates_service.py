...
from app.domain.entities import Conductor, MeteoScenario
from app.domain.value_objects import PointMeteoConditions, SegmentResults
...
def _geopoint_to_dict(p) -> dict:
    """Convierte un GeoPoint a dict serializable para la respuesta JSON."""
...
def _normalize_coordinates(coordinates: list[dict]) -> list[dict]:
    """Normaliza las coordenadas recibidas del frontend.
    Acepta tanto 'lon' como 'lng' (convención Leaflet)."""
    normalized = []
...
async def calculate_seasonal_rates(
    ...
    # Cálculo IEEE 738 por segmento y escenario
    results = []
    for i, segment in enumerate(segments):
        ...
        for season, scenario in scenarios.items():
            ...
            rating = _calculator.calcular(
                conductor=conductor,
                meteo=meteo,
                latitud_deg=segment.mid_point.lat,  # ← GeoPoint.lat
        ...
        results.append(
            {
                ...
                "mid_point": _geopoint_to_dict(segment.mid_point),  # ← serializado
                "start_point": _geopoint_to_dict(segment.start_point),
                "end_point": _geopoint_to_dict(segment.end_point),
                "rates": acc.rates,
                "details": acc.details,
