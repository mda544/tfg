...
from app.domain.seasonal_scenarios import ESCENARIOS_DEFAULT
...
# IMPORTANTE: Ahora usamos el servicio de elevación limpio con caché en BD
...
def _normalize_coordinates(coordenadas: list[dict]) -> list[dict]:
    """
    Normaliza a clave canónica 'lon'.
    ...
    for c in coordenadas:
        ...
        # Soporte para ambas nomenclaturas de altitud
        elevation = c.get("altitud") or c.get("elevation")
...
async def calculate_seasonal_rates(
    ...
    validation = validar_trazado(coordinates)
    ...
    # 3. Enriquecimiento DEM (Ahora usa la caché de PostgreSQL)
    ...
    if req.use_real_spans and len(coordinates) >= 2:
        segments     = segmentar_por_apoyos(coordinates)
        segment_mode = f"real_spans ({len(segments)} spans)"
    elif req.segment_step_m > 0:
        segments     = segmentar_trazado(coordinates, req.segment_step_m)
    ...
    for i, segment in enumerate(segments):
        ...
        for season, scenario in scenarios.items():
            ...
            acc.rates[season]   = rating.ampacidad_a
            acc.details[season] = {
                "ampacity_a":  rating.ampacidad_a,
                "qc_wm":       rating.qc_wm,
                "qr_wm":       rating.qr_wm,
                "qs_wm":       rating.qs_wm,
    ...
    if db is not None:
        ...
        # La fecha de creación es añadida directamente por PostgreSQL
