...
def _calc_length_km(coordinates) -> float:
    """Calcula la longitud del trazado en km usando Haversine."""
    total_m = sum(
        haversine_m(
...
async def create(
    ...
    entity.bbox_lat_min = min(c.lat for c in entity.coordinates)
    entity.bbox_lat_max = max(c.lat for c in entity.coordinates)
    entity.bbox_lon_min = min(c.lon for c in entity.coordinates)
    entity.bbox_lon_max = max(c.lon for c in entity.coordinates)
