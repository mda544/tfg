...
async def get_by_id(db: AsyncSession, conductor_id: str, user: UserORM) -> ConductorResponseDTO:
    try:
...
async def create(db: AsyncSession, data: ConductorCreateDTO, user: UserORM) -> ConductorResponseDTO:
    return _to_dto(await conductors_repo.create(db, user.id, data))

async def update(db: AsyncSession, conductor_id: str, data: ConductorCreateDTO, user: UserORM) -> ConductorResponseDTO:
    try:
