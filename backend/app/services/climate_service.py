...
from backend.app.domain.climate_processor import ClimateProcessor
...
async def get_climate_percentiles(
    ...
    if cached:
        ...
        return ClimatePercentilesResponseDTO(
            source=source,
            point=GeoPointDTO(lat=lat, lon=lon),  # se devolvuelven las originales
    ...
    if source == "nasa":
        raw = await NasaPowerClient().fetch_daily_data(
            lat_r, lon_r, f"{year_start}-01-01", f"{year_end}-12-31"
        )
