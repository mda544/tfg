...
class CacheRepository:
    def _round_coords(self, lat: float, lon: float) -> tuple[float, float]:
        ...
        Mantiene la precisión real del satélite sin crear cachés duplicadas.
        """
    ...
    # --- CLIMATE CACHE ---
    async def get_climate(self, db: AsyncSession, lat: float, lon: float, source: str) -> dict[Season, SeasonalPercentiles] | None:
        ...
        return {
            row.season: SeasonalPercentiles(
                season=row.season, lat=row.lat, lon=row.lon,
                temp_p90_c=row.temp_p90_c, temp_p50_c=row.temp_p50_c, temp_p10_c=row.temp_p10_c,
                wind_p10_ms=row.wind_p10_ms, wind_p50_ms=row.wind_p50_ms, wind_p90_ms=row.wind_p90_ms,
                radiation_p50_wm2=row.radiation_p50_wm2, radiation_p90_wm2=row.radiation_p90_wm2,
                n_hours=row.n_hours, source=row.source, years_covered=row.years_covered
            ) for row in rows
        }

    async def save_climate(self, db: AsyncSession, percentiles: dict[Season, SeasonalPercentiles]) -> None:
        for season, p in percentiles.items():
            lat_r, lon_r = self._round_coords(p.lat, p.lon)
            stmt = insert(ClimateCacheORM).values(
                lat=lat_r, lon=lon_r, source=p.source, season=season,
                temp_p90_c=p.temp_p90_c, temp_p50_c=p.temp_p50_c, temp_p10_c=p.temp_p10_c,
                ...
                n_hours=p.n_hours, years_covered=p.years_covered
    ...
    async def save_elevation(self, db: AsyncSession, lat: float, lon: float, elevation_m: float) -> None:
