...
class ClimateProcessor:
    ...
    def process_openmeteo_data(
        ...
        return ClimateProcessor._compute_percentiles(
            lat, lon, months, temps, winds, radiation,
    ...
    def _compute_percentiles(
        lat: float, lon: float,
        months: np.ndarray, temps: np.ndarray,
        winds: np.ndarray, radiation: np.ndarray,
        source: str, years: str,
    ) -> dict[Season, SeasonalPercentiles]:
        ...
        for season, season_months in SEASON_MONTHS.items():
            ...
            results[season] = SeasonalPercentiles(
                ...
                radiation_p50_wm2 = round(float(np.percentile(r_daytime, 50)) if len(r_daytime) > 0 else 0.0, 1),
                radiation_p90_wm2 = round(float(np.percentile(r_daytime, 90)) if len(r_daytime) > 0 else 0.0, 1),
