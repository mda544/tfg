...
_FUENTE_DEM = "openmeteo_dem_v1"   # versionar la fuente permite invalidar caché fácilmente


# Caché en disco

def _clave_cache(lat: float, lon: float, fuente: str = _FUENTE_DEM) -> str:
    lat_r = round(lat, 3)
    lon_r = round(lon, 3)
    return hashlib.md5(f"{lat_r}_{lon_r}_{fuente}".encode()).hexdigest()[:10]
...
def _escribir_cache(lat: float, lon: float, altitud_m: float) -> None:
    ruta = CACHE_DIR / f"{_clave_cache(lat, lon)}.json"
    try:
        ruta.write_text(json.dumps({"lat": lat, "lon": lon, "altitud_m": altitud_m, "fuente": _FUENTE_DEM}))
    ...
    async with httpx.AsyncClient(timeout=15.0) as client:
        for i in range(0, len(puntos), 100):
            ...
            try:
                ...
                resultados.extend(float(e) if e is not None else None for e in elevaciones)
            except Exception as e:
...
async def obtener_altitudes_trazado(coordenadas: list[dict]) -> list[float]:
    n = len(coordenadas)
    altitudes = [0.0] * n
    pendientes_idx: list[int]              = []
    pendientes_pts: list[tuple[float, float]] = []
    ...
    aun_pendientes: list[tuple[int, tuple[float, float]]] = []
...
async def enriquecer_coordenadas_con_dem(coordenadas: list[dict]) -> list[dict]:
