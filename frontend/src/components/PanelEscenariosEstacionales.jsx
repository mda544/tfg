...
const META = {
  verano:    { label: "Verano",    descripcion: "Condición más restrictiva",        color: "coral"  },
  invierno:  { label: "Invierno",  descripcion: "Mayor capacidad de transporte",    color: "blue"   },
  primavera: { label: "Primavera", descripcion: "Condición intermedia",             color: "green"  },
  otono:     { label: "Otoño",     descripcion: "Condición intermedia",             color: "amber"  },
};

// ── IEEE 738 simplificado para preview en tiempo real ──────────────────────
// El cálculo definitivo lo hace el backend; este es solo orientativo en la UI
function calcularAmpacidadPreview(escenario, conductorRef) {
  const { diametro_mm = 28.1, r_ac_75 = 0.072, temp_max = 90 } = conductorRef ?? {};
  ...
  const kAng = 1.194 - Math.cos(phi) + 0.194 * Math.cos(2 * phi) + 0.368 * Math.sin(2 * phi);
  ...
  const qr = 0.5 * Math.PI * D * sigma * ((Tc + 273.15) ** 4 - (Ta + 273.15) ** 4);
...
// ── Subcomponente: campo con slider + valor ────────────────────────────────
const CampoSlider = ({ label, unidad, nombre, valor, min, max, paso, onChange }) => (
...
const PanelEscenariosEstacionales = ({ conductorRef, onChange }) => {
  const [activa, setActiva] = useState("verano");
  const [escenarios, setEscenarios] = useState(() =>
    Object.fromEntries(ESTACIONES.map((e) => [e, { ...DEFAULTS[e] }]))
  );

  const actualizarCampo = useCallback(
    (campo, valor) => {
      setEscenarios((prev) => {
        const nuevo = { ...prev, [activa]: { ...prev[activa], [campo]: valor } };
        onChange?.(nuevo);  // notifica al padre (App.jsx) en cada cambio
        return nuevo;
      });
    },
    [activa, onChange]
  ...
  const resetearEstacion = () => {
    setEscenarios((prev) => {
      const nuevo = { ...prev, [activa]: { ...DEFAULTS[activa] } };
      onChange?.(nuevo);
      return nuevo;
    });
  };
  ...
  const condViento =
    escenarioActivo.viento < 1 ? "Calma — caso crítico"
    : escenarioActivo.viento < 3 ? "Moderado"
    : "Fuerte";
  ...
  return (
    <div className="pes-panel">
      ...
      <CampoSlider
        label="Temperatura ambiente"  unidad="°C"
        nombre="temp"   valor={escenarioActivo.temp}
        min={-10} max={50} paso={1}
        onChange={actualizarCampo}
      />
      <CampoSlider
        label="Velocidad de viento"  unidad="m/s"
        nombre="viento"  valor={escenarioActivo.viento}
        min={0} max={15} paso={0.1}
        onChange={actualizarCampo}
      />
      <CampoSlider
        label="Radiación solar"  unidad="W/m²"
        nombre="radiacion" valor={escenarioActivo.radiacion}
        min={0} max={1200} paso={10}
        onChange={actualizarCampo}
      />
      <CampoSlider
        label="Ángulo viento / conductor"  unidad="°"
        nombre="angulo"  valor={escenarioActivo.angulo}
        min={0} max={90} paso={1}
        onChange={actualizarCampo}
      />
      ...
      <div className="pes-metricas">
        ...
        <div className="pes-metrica">
          ...
          <span className="pes-metrica-valor" style={{ fontSize: "14px", marginTop: "4px" }}>
            {condViento}
          </span>
        </div>
      </div>
      ...
      <div className="pes-acciones">
        ...
        <button className="pes-btn pes-btn--secundario" onClick={() => resetearTodo(setEscenarios, onChange)}>
          Restaurar todos
        </button>
      </div>
...
function resetearTodo(setEscenarios, onChange) {
  const reset = Object.fromEntries(ESTACIONES.map((e) => [e, { ...DEFAULTS[e] }]));
  setEscenarios(reset);
  onChange?.(reset);
}
...
// Exporta también los defaults para usarlos en el payload del backend
