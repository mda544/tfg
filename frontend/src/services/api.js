...
const URL_BACKEND = "http://192.168.1.36:8000";
...
export const enviarCalculoRendimiento = async (paqueteDatos) => {
  try {
    ...
    if (error.request) {
      throw new Error(
        "No se pudo contactar con el servidor. Revisa si el backend está activo.",
      );
    }
...
export const obtenerClimatologiaHistorica = async (
  lat,
  lon,
  fuente = "openmeteo",
) => {
  try {
    const respuesta = await axios.get(
      `${URL_BACKEND}/climatologia/percentiles`,
      {
    ...
    if (respuesta.data.status === "error") {
      throw new Error(
        respuesta.data.mensaje || "Fallo interno en el cálculo (Python)",
      );
    }
